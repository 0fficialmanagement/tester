...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- Advanced Phone Input with intl-tel-input ---
  const phoneError = document.getElementById("phone-error");
  if (window.intlTelInput && phoneInput) {
    const iti = window.intlTelInput(phoneInput, {
      ...
      geoIpLookup: function(callback) {
        fetch('https://ipwho.is/')
          .then(res => res.json())
          .then(data => callback(data.country_code ? data.country_code : 'us'))
          .catch(() => callback('us'));
      },
    ...
    // Sync country code to hidden input if needed
    phoneInput.addEventListener('countrychange', function() {
      // Optionally update other fields based on country
    });
  }

  // --- FORM SUBMISSION HANDLER (AJAX/Netlify + ModalManager) ---
  if (form) {
    form.addEventListener('submit', async function(e) {
      e.preventDefault();
      if (!form.checkValidity()) {
        modalManager.show('globalErrorModal');
      ...
      try {
        ...
        if (netlifyError) console.warn('Netlify submission error occurred');
      }
    });
  }
  ...
  // Use only this for global errors
  ...
  // Initialize Choices.js for country, branch, group, and artist dropdowns
  if (window.Choices) {
    if (countrySelect) {
      ...
      countrySelect.addEventListener('change', function () {
        if (countryInput) countryInput.value = countrySelect.value;
      });
    }
    if (branchSelect) {
    ...
    if (artistSelect) {
      new Choices(artistSelect, {
        ...
        allowHTML: false,
        placeholder: true,
        placeholderValue: 'Select Artist',
        classNames: { containerOuter: 'choices form-select' },
      });
    }
  }
